...
async function startHisoka() {
    ...
    hisoka.sendMessage(`6283127014833@s.whatsapp.net`, {text: `*Lapor Bot:* Ada Yang Menelpon Bot`})
    await sleep(8000)
...
Kon = await getBuffer(`https://hardianto.xyz/api/welcome3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://www.linkpicture.com/q/20211125_113317.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)

Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://www.linkpicture.com/q/20211125_113317.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                if (anu.action == 'add') {
                    hisoka.sendMessage(anu.id, { image: Kon, contextInfo: { mentionedJid: [num] }, caption: `Welcome To ${metadata.subject} @${num.split("@")[0]}

Descripsi: ${metadata.desc}
Semoga Betah Kak ><`} )
